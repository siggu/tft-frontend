...
export default function ChampionDetail() {
  ...
  return (
    <VStack gap={0} justifyContent={"center"} pl="10%" pr="10%" fontSize={"sm"}>
        ...
        {/* 이름, 초상화, 비용, 계열, 직업 */}
      <HStack width="100%" justifyContent={"space-between"} border={"solid"} borderColor='gray.300' borderWidth={1} gap={5} borderBottom={0} backgroundColor="black" color="white">
        <HStack p={5}>
          <Image src={data?.photos[0].file} borderRadius={"20"} />
          <Text p={5} fontSize={"2xl"} as={"b"}>{data?.name}</Text>
        </HStack>
        ...
        <HStack >
          {data?.origin.length === 1 ? (
            <>
              <Image w="45px" h="45px"rounded={"2xl"} bgColor={"gray"}
      ...
      <HStack width="100%" alignItems={"flex-start"}border={"solid"} 
              borderColor='gray.300' borderWidth={1} p={5} gap={5} 
              borderBottom={0} justifyContent={"space-evenly"}>
        <VStack>
          <Text>체력</Text>
