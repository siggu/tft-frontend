...
export default function Profile() {
  ...
  const { data: leagueEntryData, isLoading: isLeagueEntryDataLoading } =
    useQuery<ILeagueEntryDTO>({
      queryKey: ["", summonerId],
      queryFn: getLeagueEntries,
    });
  const { data: matchesByPuuidData, isLoading: ismatchByPuuidDataLoading } =
    useQuery<ILeagueEntryDTO>({
      queryKey: ["", puuid],
                  ...
                  <>
                    <Box mb={10} border={"1px solid white"} p={5}>
                      ...
                      <Box border={"1px solid white"} p={3}>
                        ...
                        {match.info.participants[index].augments.map(
                          (augment) => (
                      ...
                      <Box border={"1px solid white"} p={3}>
                        <Text color={"white"}>
                          전설이:{" "}
                          {match.info.participants[index].companion.content_ID}
                        </Text>
                      </Box>
                      {/* 남은 골드 */}
                      <Box border={"1px solid white"} p={3}>
                        <Text color={"white"}>
                          남은 골드: {match.info.participants[index].gold_left}
                        </Text>
                      </Box>
                      {/* 레벨 */}
                      <Box border={"1px solid white"} p={3}>
                        <Text color={"white"}>
                          레벨: {match.info.participants[index].level}
                        </Text>
                      </Box>
                      {/* 등수 */}
                      <Box border={"1px solid white"} p={3}>
                        <Text color={"white"}>
                          등수: {match.info.participants[index].placement}
                        </Text>
                      </Box>
                      {/* 처치한 플레이어 수 */}
                      <Box border={"1px solid white"} p={3}>
                        <Text color={"white"}>
                          처치한 플레이어 수:{" "}
                          {match.info.participants[index].players_eliminated}
                        </Text>
                      </Box>
                      {/* 플레이어에게 가한 대미지 */}
                      <Box border={"1px solid white"} p={3}>
                      ...
                      <Box border={"1px solid white"} p={3}>
                        ...
                        {match.info.participants[index].units.map(
                          (champion) => (
                            <>
                              <Box border={"1px solid white"} p={3}>
                                <Text color={"white"}>
